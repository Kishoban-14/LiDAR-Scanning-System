...
void scan360_and_send_uart(void) {
    ...
    while (motor_status) {  // Run continuously until PJ1 is pressed to stop the motor
        for (int i = 0; i < 8; i++) {
						FlashLED1(10); //Blink LED for additional status and bus speed verification PF4/LED D1
            if (data_acquisition) {  // Only acquire data if enabled
                while (dataReady == 0) {
                    VL53L1X_CheckForDataReady(dev, &dataReady);
                    FlashLED3(1);  // Blink LED to indicate measurement is in progress via I2C PN0/LED D3
                    VL53L1_WaitMs(dev, 1);
                }
                dataReady = 0;
                VL53L1X_GetDistance(dev, &Distance);
                FlashLED4(1);   //Blink LED when measurement is sent via UART PN1/LED D4
                VL53L1X_ClearInterrupt(dev);

                double angle_deg = i * 45;
                double angle_rad = angle_deg * (PI / 180.0);
                double x = x_offset;
                double y = Distance * cos(angle_rad); //Trig functions
